const APP_ROLE = document.documentElement.dataset.cpgRole || 'combined';
const STATE_KEY = APP_ROLE==='planner' ? 'cpg-inkasacja-planner-state' : APP_ROLE==='executor' ? 'cpg-inkasacja-executor-state' : 'cpg-inkasacja-state';
const LEGACY_STATE_KEY = 'cpg-inkasacja-state';
const CLIENT_ID_KEY = 'cpg-executor-client-id';
function randomClientId(){
  const bytes=new Uint8Array(12);crypto.getRandomValues(bytes);return 'cpg_'+Array.from(bytes,b=>b.toString(16).padStart(2,'0')).join('');
}
function getClientId(){
  let id=sessionStorage.getItem(CLIENT_ID_KEY);
  if(!id){id=randomClientId();sessionStorage.setItem(CLIENT_ID_KEY,id)}
  return id;
}
const EXECUTOR_CLIENT_ID=getClientId();
...
const state = {
  ...
  syncPaused:false, leaseBlocked:false, leaseTimer:null, plannerAdminToken:null,
  sealSequence:{lastUsed:null,nextSuggested:null,lastIndex:null},
  convoy:{id:null,date:null,status:'draft',createdAt:null,publishedAt:null,updatedAt:null,cloudProvider:null,cloudItemId:null,cloudETag:null,shareToken:null,revision:1,lastWriterDeviceId:null},
...
async function preparePlan(){
 ...
 state.convoy={id:`convoy-${convoyDate}`,date:convoyDate,status:'draft',createdAt:now,publishedAt:null,updatedAt:now,cloudProvider:null,cloudItemId:null,cloudETag:null,shareToken:null,revision:1,lastWriterDeviceId:null};
...
function localStateObject(){return {plan:state.plan,route:state.route,start:state.start,routeMeta:state.routeMeta,history:state.history,mode:state.mode,convoy:state.convoy,sourceMeta:state.sourceMeta,reportSnapshot:state.reportSnapshot,sealSequence:state.sealSequence}}
function persistLocalState(){localStorage.setItem(STATE_KEY,JSON.stringify(localStateObject()))}
function saveState(){persistLocalState(); if(!state.suppressCloudSync)scheduleCloudSave()}
function clearState(){localStorage.removeItem(STATE_KEY)}
function restoreState(){try{const raw=localStorage.getItem(STATE_KEY)||localStorage.getItem(LEGACY_STATE_KEY);const x=raw?JSON.parse(raw):null;if(x?.route?.length){state.plan=(x.plan||[]).map(r=>({address:r.address||r.location||'Brak adresu',...r}));state.route=x.route.map(r=>({collectedCash:null,qrRaw:'',qrScannedAt:null,address:r.address||r.location||'Brak adresu',...r}));state.start=x.start||state.start;state.routeMeta=x.routeMeta||{startedAt:null,finishedAt:null};state.history=x.history||[];state.mode=x.mode||null;state.convoy={...state.convoy,...(x.convoy||{})};state.sourceMeta=x.sourceMeta||null;state.reportSnapshot=x.reportSnapshot||null;state.sealSequence={...state.sealSequence,...(x.sealSequence||{})};}}catch{}}
...
function renderRoute(){syncWorkflowUi();
 ...
 const plannerView=state.mode==='planner';
 $('statSelected').textContent=state.route.length;
 if($('statCash')){ $('statCash').textContent=plannerView?money(state.route.reduce((s,x)=>s+(Number(x.cash)||0),0)):''; $('statCash').closest('.stat')?.classList.toggle('hidden',!plannerView); }
 if($('statCollectedCash')) $('statCollectedCash').textContent=money2(state.route.reduce((s,x)=>s+(x.status==='done'?(Number(x.collectedCash)||0):0),0)); $('statDone').textContent=done; $('statSkipped').textContent=skipped;
 $('routeTitle').textContent=plannerView?`Plan • ${state.route.length} urządzeń`:`Konwój • ${state.route.length} urządzeń`;
 ...
 $('deviceList').innerHTML=visible.map(({x,i})=>{const amount=plannerView?money(x.cash):(x.status==='done'&&x.collectedCash!==null?`<div class="collected-value">wybrano: ${money2(x.collectedCash)}</div>`:'');return `<div class="device-row ${i===nextIndex?'next-pending':''}" data-i="${i}"><div class="order">${i+1}</div><div><strong>${escapeHtml(x.id)}</strong><div class="sub">${escapeHtml(x.location)}${x.address&&x.address!==x.location?` • ${escapeHtml(x.address)}`:''}</div><span class="badge ${x.status==='done'?'done':x.status==='skip'?'skip':'pending'}">${x.status==='done'?'Zainkasowano':x.status==='skip'?'Nie zainkasowano':i===nextIndex?'Następny':'Do wykonania'}</span></div><div class="amount">${amount}</div></div>`}).join('')||'<div class="card muted">Brak urządzeń spełniających filtr.</div>';
...
function renderNextStop(){
  ...
  $('nextStopId').textContent=`${i+1}. ${x.id}`; $('nextStopLocation').textContent=x.location; if($('nextStopCash'))$('nextStopCash').textContent=state.mode==='planner'?money(x.cash):'';
}
...
function openDevice(i){if(state.mode==='planner'){toast('Planista może przeglądać trasę. Dane inkasa uzupełnia konwojent po rozpoczęciu konwoju.');return} if(state.leaseBlocked){toast('Konwój jest obecnie używany na innym urządzeniu. Wczytaj najnowszą wersję.');return} if(!state.routeMeta.startedAt){toast('Najpierw rozpocznij konwój.');return} state.currentIndex=i; const x=state.route[i]; $('deviceIndex').textContent=`URZĄDZENIE ${i+1} Z ${state.route.length}`; $('deviceId').textContent=x.id; $('deviceLocation').textContent=x.address&&x.address!==x.location?`${x.location} • ${x.address}`:x.location; if($('deviceCash')){ $('deviceCash').textContent=''; $('deviceCash').closest('span')?.classList.add('hidden'); } setSealFieldForDevice(i); if($('collectedCash')) $('collectedCash').value=(x.collectedCash===null||x.collectedCash===undefined)?'':String(x.collectedCash).replace('.',','); $('notes').value=x.notes||''; $('skipReason').value=x.reason||'Brak możliwości dojazdu'; setChoice(x.status==='skip'?'skip':'done'); updateQrStatus(x); $('undoDeviceBtn')?.classList.toggle('hidden',!state.history.some(h=>h.index===i)); showView('deviceView')}
...
function saveCurrentDevice(){
 if(state.leaseBlocked||state.syncPaused){alert('Konwój jest zablokowany do edycji, ponieważ wykryto inne urządzenie lub nowszą wersję danych. Wczytaj najnowszą wersję.');return false}
...
$('confirmFinishBtn').onclick=async()=>{
  ...
  let cloudSaved=true;
  try{await flushCloudSave({throwOnError:true});await releaseExecutorLease()}catch(e){cloudSaved=false;console.error(e)}
...
function resetAll(){if(confirm('Wyczyścić dane lokalne na tym urządzeniu? Opublikowany plan w chmurze nie zostanie usunięty.')){releaseExecutorLease();state.plan=[];state.route=[];state.sourceRows=[];state.lastExcelBlob=null;state.mode=APP_ROLE==='planner'?'planner':APP_ROLE==='executor'?'executor':null;state.sourceMeta=null;state.reportSnapshot=null;state.syncPaused=false;state.leaseBlocked=false;state.sealSequence={lastUsed:null,nextSuggested:null,lastIndex:null};state.convoy={id:null,date:null,status:'draft',createdAt:null,publishedAt:null,updatedAt:null,cloudProvider:null,cloudItemId:null,cloudETag:null,shareToken:null,revision:1,lastWriterDeviceId:null};state.routeMeta={startedAt:null,finishedAt:null};clearState();if($('fileStatus'))$('fileStatus').textContent='Nie wczytano pliku.';showView(APP_ROLE==='planner'?'setupView':APP_ROLE==='executor'?'convoyLoadView':'homeView')}}
...
function setPlannerMode(){if(APP_ROLE==='executor')return;state.mode='planner';setDefaultDates();showView('setupView')}
function setExecutorMode(){if(APP_ROLE==='planner')return;state.mode='executor';setDefaultDates();showView('convoyLoadView')}
$('plannerModeBtn')?.addEventListener('click',setPlannerMode);
$('executorModeBtn')?.addEventListener('click',setExecutorMode);
$('plannerBackBtn')?.addEventListener('click',()=>showView(APP_ROLE==='planner'?'setupView':'homeView'));
$('executorBackBtn')?.addEventListener('click',()=>showView(APP_ROLE==='executor'?'convoyLoadView':'homeView'));
...
function sanitizeRouteItemForCloud(x){
  return {
    id:x?.id||'', location:x?.location||'', address:x?.address||x?.location||'',
    lat:Number(x?.lat)||0, lng:Number(x?.lng)||0,
    status:x?.status||'pending', seal:x?.seal||'', reason:x?.reason||'', notes:x?.notes||'',
    updatedAt:x?.updatedAt||null, collectedCash:x?.collectedCash===null||x?.collectedCash===undefined?null:Number(x.collectedCash)||0,
    qrRaw:x?.qrRaw||'', qrScannedAt:x?.qrScannedAt||null
  };
}
function cloudPayload(){
  const route=(state.route||[]).map(sanitizeRouteItemForCloud);
  const convoy={...state.convoy,revision:Number(state.convoy?.revision||1)};
  return {schema:'cpg-inkasacja-v8',exportedAt:new Date().toISOString(),convoy,start:state.start,plan:route.map(x=>({...x})),route,routeMeta:state.routeMeta,history:[],sealSequence:state.sealSequence,reportSnapshot:state.reportSnapshot,sourceMeta:null};
}
function convoyShareUrl(date=state.convoy.date,token=state.convoy.shareToken){
 const configured=String(window.CPG_GOOGLE_CONFIG?.executorUrl||'').trim();
 const u=configured?new URL(configured,window.location.href):new URL(window.location.href);
 u.hash='';u.search='';u.searchParams.set('convoy',date||'');u.searchParams.set('token',token||'');return u.toString();
}
...
function syncWorkflowUi(){
 ...
 if($('executorPlanStatus'))$('executorPlanStatus').textContent=state.leaseBlocked?'Tryb tylko do odczytu — konwój jest aktywny na innym urządzeniu.':(state.convoy.status==='in_progress'?'Konwój był już rozpoczęty. Możesz kontynuować.':'Plan opublikowany i gotowy do realizacji.');
 if($('startConvoyBtn'))$('startConvoyBtn').disabled=!!state.leaseBlocked;
...
async function publishConvoy(){
 ...
 try{
   ...
   const published=await window.CPG_GOOGLE.publishConvoy(cloudPayload(),pin,token,(info)=>{
   ...
   state.convoy.revision=Number(published?.meta?.revision||1);state.convoy.lastWriterDeviceId='planner';persistLocalState();
   setCloudSyncStatus('Plan opublikowany w Google.','ok');syncWorkflowUi();
   finishPublishProgressTimer();
...
function plannerHistoryStatusLabel(status){return status==='completed'?'Zakończony':status==='in_progress'?'W realizacji':status==='published'?'Opublikowany':'Szkic'}
function renderPlannerHistory(items){
 const list=$('plannerHistoryList');if(!list)return;
 if(!items?.length){list.innerHTML='<div class="muted">Brak opublikowanych konwojów.</div>';return}
 list.innerHTML=items.map((x,i)=>{const link=convoyShareUrl(x.date,x.token);return `<div class="planner-history-row"><div><strong>${escapeHtml(x.date||'—')}</strong><small>${escapeHtml(plannerHistoryStatusLabel(x.status))} • ${Number(x.done||0)}/${Number(x.count||0)} wykonane${x.finishedAt?` • zakończono ${escapeHtml(formatDateTime(x.finishedAt))}`:''}</small></div><div class="planner-history-actions"><button class="secondary compact" data-history-copy="${i}">Kopiuj link</button><a class="primary compact button-link" href="${escapeHtml(link)}" target="_blank" rel="noopener noreferrer">Otwórz</a></div></div>`}).join('');
 list.querySelectorAll('[data-history-copy]').forEach(btn=>btn.addEventListener('click',async()=>{const x=items[Number(btn.dataset.historyCopy)];const link=convoyShareUrl(x.date,x.token);try{await navigator.clipboard.writeText(link);toast('Skopiowano link konwoju.')}catch{prompt('Skopiuj link:',link)}}));
}
async function refreshPlannerHistory(){
 if(!window.CPG_GOOGLE?.isConfigured?.()){alert('Google Apps Script nie jest skonfigurowany.');return}
 const pin=prompt('Podaj PIN planisty, aby pobrać historię opublikowanych konwojów:');if(pin===null)return;if(!String(pin).trim()){toast('Wpisz PIN planisty.');return}
 const status=$('plannerHistoryStatus');if(status)status.textContent='Autoryzacja i pobieranie listy…';
 try{const login=await window.CPG_GOOGLE.plannerLogin(String(pin).trim());state.plannerAdminToken=login.adminToken;const items=await window.CPG_GOOGLE.listConvoys(login.adminToken);renderPlannerHistory(items);if(status)status.textContent=`Pobrano ${items.length} konwojów.`;}catch(e){if(status)status.textContent='Nie udało się pobrać historii.';alert(e.message)}
}
$('refreshPlannerHistoryBtn')?.addEventListener('click',refreshPlannerHistory);

async function claimExecutorLease(){
 if(state.mode!=='executor'||isCompletedConvoy()||state.convoy.cloudProvider!=='google'||!state.convoy.shareToken||!window.CPG_GOOGLE?.isConfigured?.())return true;
 try{
   const lease=await window.CPG_GOOGLE.claimConvoy(state.convoy.date,state.convoy.shareToken,EXECUTOR_CLIENT_ID);
   if(!lease?.claimed){
     state.leaseBlocked=true;state.syncPaused=true;
     setCloudSyncStatus(`Konwój jest otwarty na innym urządzeniu${lease?.ownerHint?` (${lease.ownerHint})`:''}.`,'warning',true);
     syncWorkflowUi();return false;
   }
   state.leaseBlocked=false;state.syncPaused=false;startLeaseRenewal();syncWorkflowUi();return true;
 }catch(e){
   console.error(e);
   if(!navigator.onLine&&state.route?.length){state.leaseBlocked=false;state.syncPaused=false;setCloudSyncStatus('Brak Internetu — pracujesz na danych zapisanych na tym urządzeniu. Zmiany zsynchronizują się po powrocie sieci.','offline');syncWorkflowUi();return true}
   state.leaseBlocked=true;state.syncPaused=true;setCloudSyncStatus('Nie udało się potwierdzić wyłącznego dostępu do konwoju.','warning',true);syncWorkflowUi();return false;
 }
}
function startLeaseRenewal(){
 if(state.leaseTimer)clearInterval(state.leaseTimer);
 if(state.mode!=='executor'||isCompletedConvoy())return;
 state.leaseTimer=setInterval(async()=>{
   if(!navigator.onLine||state.mode!=='executor'||isCompletedConvoy())return;
   try{const lease=await window.CPG_GOOGLE.claimConvoy(state.convoy.date,state.convoy.shareToken,EXECUTOR_CLIENT_ID);if(!lease?.claimed){state.leaseBlocked=true;state.syncPaused=true;setCloudSyncStatus('Konwój przejęło inne urządzenie. Wczytaj najnowszą wersję.','warning',true);syncWorkflowUi()}}
   catch(e){console.error(e)}
 },60000);
}
async function releaseExecutorLease(){
 if(state.leaseTimer){clearInterval(state.leaseTimer);state.leaseTimer=null}
 if(state.convoy.cloudProvider==='google'&&state.convoy.date&&state.convoy.shareToken&&window.CPG_GOOGLE?.isConfigured?.()){
   try{await window.CPG_GOOGLE.releaseConvoy(state.convoy.date,state.convoy.shareToken,EXECUTOR_CLIENT_ID)}catch{}
 }
}
function pendingSyncKey(){return state.convoy?.date&&state.convoy?.shareToken?`cpg-pending-sync:${state.convoy.date}:${state.convoy.shareToken}`:''}
function markPendingCloud(payload,baseRevision){const k=pendingSyncKey();if(k)localStorage.setItem(k,JSON.stringify({payload,baseRevision:Number(baseRevision||1),queuedAt:new Date().toISOString()}))}
function clearPendingCloud(){const k=pendingSyncKey();if(k)localStorage.removeItem(k)}
function hasPendingCloud(){const k=pendingSyncKey();return !!(k&&localStorage.getItem(k))}
...
async function loadConvoyPayload(data,cloudItem=null){
 ...
 try{
  ...
  const revision=Number(cloudItem?.revision||data.convoy?.revision||1)||1;
  state.mode='executor';state.lastExcelBlob=null;state.plan=(data.plan||data.route||[]).map(r=>({...r,address:r.address||r.location||'Brak adresu'}));state.route=(data.route||[]).map(r=>({...r,address:r.address||r.location||'Brak adresu'}));state.start=data.start||state.start;state.routeMeta=data.routeMeta||{startedAt:null,finishedAt:null};state.history=data.history||[];state.convoy={...state.convoy,...(data.convoy||{}),revision,lastWriterDeviceId:cloudItem?.lastWriterDeviceId||data.convoy?.lastWriterDeviceId||null,cloudProvider:provider,shareToken:token,cloudItemId:provider==='google'?`google:${data.convoy?.date||''}:${token||''}`:(cloudItem?.id||data.convoy?.cloudItemId||null),cloudETag:cloudItem?.eTag||data.convoy?.cloudETag||null};state.sourceMeta=null;state.reportSnapshot=data.reportSnapshot||null;state.sealSequence=data.sealSequence?{lastUsed:null,nextSuggested:null,lastIndex:null,...data.sealSequence}:{lastUsed:null,nextSuggested:null,lastIndex:null};state.syncPaused=false;state.leaseBlocked=false;if(!state.sealSequence.nextSuggested)rebuildSealSequence();if(state.routeMeta?.finishedAt)state.convoy.status='completed';if(isCompletedConvoy()&&!state.reportSnapshot)state.reportSnapshot=buildReportSnapshot();persistLocalState();renderRoute();
  if(isCompletedConvoy()){await releaseExecutorLease();renderCompletedReportView({reopened:true});showView('reportView');setCloudSyncStatus('Konwój zakończony — dane są zapisane w Google.','ok')}
  else{showView('routeView');await claimExecutorLease();if(!state.leaseBlocked)setCloudSyncStatus('Połączono z Google — zapis automatyczny aktywny.','ok')}
...
async function loadConvoyByDate(){
 ...
 try{
   ...
   await loadConvoyPayload(result.data,{provider:'google',token,revision:result.meta?.revision,lastWriterDeviceId:result.meta?.lastWriterDeviceId});
   $('convoyLoadStatus').textContent=isCompletedConvoy()?'Zakończony konwój pobrany. Raport jest dostępny do ponownego pobrania.':state.leaseBlocked?'Plan pobrany w trybie tylko do odczytu — konwój jest otwarty na innym urządzeniu.':'Plan pobrany.';
...
async function autoLoadSharedConvoyFromUrl(){
 ...
 setTimeout(()=>loadConvoyByDate(),250);return true;
}
...
async function startConvoy(){
 if(!state.route.length)return;
 if(state.leaseBlocked){alert('Ten konwój jest obecnie otwarty na innym urządzeniu. Nie można rozpocząć realizacji.');return}
 if(!await claimExecutorLease())return;
...
function setCloudSyncStatus(msg,kind='neutral',showReload=false){
 if($('cloudSyncStatus'))$('cloudSyncStatus').textContent=msg;
 const bar=$('cloudSyncBar');if(bar){bar.classList.remove('sync-ok','sync-saving','sync-warning','sync-error','sync-offline');if(kind&&kind!=='neutral')bar.classList.add(`sync-${kind}`)}
 $('reloadCloudBtn')?.classList.toggle('hidden',!showReload);
}
function scheduleCloudSave(){
 ...
 const baseRevision=Number(state.convoy?.revision||1);const payload=cloudPayload();markPendingCloud(payload,baseRevision);
 clearTimeout(state.cloudSyncTimer);
 if(isCompletedConvoy())return;
 if(state.syncPaused||state.leaseBlocked){setCloudSyncStatus('Zmiana czeka. Wymagane wczytanie najnowszej wersji.','warning',true);return}
 if(!navigator.onLine){setCloudSyncStatus('Brak Internetu — zmiana zapisana na telefonie i czeka na synchronizację.','offline');return}
 setCloudSyncStatus('Zmiana czeka na zapis w Google…','saving');state.cloudSyncTimer=setTimeout(()=>flushCloudSave(),700);
}
async function flushCloudSave(options={}){
 if(state.suppressCloudSync||state.mode!=='executor')return true;
 if(isCompletedConvoy()&&!hasPendingCloud())return true;
 const google=state.convoy.cloudProvider==='google'&&state.convoy.shareToken&&window.CPG_GOOGLE?.isConfigured?.();
 const m365=state.convoy.cloudProvider==='m365'&&state.convoy.cloudItemId&&window.CPG_M365?.isConfigured?.();
 if(!google&&!m365)return true;
 if(!navigator.onLine){setCloudSyncStatus('Brak Internetu — dane czekają na synchronizację.','offline');return false}
 if(state.syncPaused||state.leaseBlocked){setCloudSyncStatus('Synchronizacja wstrzymana — wczytaj najnowsze dane.','warning',true);return false}
 try{
   clearTimeout(state.cloudSyncTimer);state.cloudSyncTimer=null;state.convoy.updatedAt=new Date().toISOString();
   const baseRevision=Number(state.convoy?.revision||1);const payload=cloudPayload();markPendingCloud(payload,baseRevision);setCloudSyncStatus('Zapisuję zmiany w Google…','saving');
   if(google){
     const lease=await window.CPG_GOOGLE.claimConvoy(state.convoy.date,state.convoy.shareToken,EXECUTOR_CLIENT_ID);
     if(!lease?.claimed){const err=new Error('Konwój jest obecnie używany na innym urządzeniu.');err.code='LEASE_CONFLICT';throw err}
     const meta=await window.CPG_GOOGLE.saveConvoy(state.convoy.date,state.convoy.shareToken,payload,{deviceId:EXECUTOR_CLIENT_ID,baseRevision});
     state.convoy.revision=Number(meta?.revision||baseRevision+1);state.convoy.lastWriterDeviceId=EXECUTOR_CLIENT_ID;persistLocalState();clearPendingCloud();setCloudSyncStatus(`Zapisano w Google ${new Date().toLocaleTimeString('pl-PL',{hour:'2-digit',minute:'2-digit'})}`,'ok');
   }
   else{const saved=await window.CPG_M365.saveConvoy(convoyFileName(),sessionPayload());state.convoy.cloudItemId=saved.id||state.convoy.cloudItemId;state.convoy.cloudETag=saved.eTag||saved.etag||state.convoy.cloudETag;clearPendingCloud();setCloudSyncStatus(`Zapisano ${new Date().toLocaleTimeString('pl-PL',{hour:'2-digit',minute:'2-digit'})}`,'ok');}
   return true;
 }catch(e){
   console.error(e);
   if(e?.code==='CONFLICT'||e?.code==='LEASE_CONFLICT'){state.syncPaused=true;state.leaseBlocked=true;setCloudSyncStatus('Wykryto nowsze dane lub aktywne inne urządzenie. Wczytaj najnowszą wersję.','warning',true);syncWorkflowUi()}
   else setCloudSyncStatus('Nie udało się zapisać — dane są bezpiecznie zapisane na tym telefonie i czekają na Internet.','error');
   if(options.throwOnError)throw e;return false
 }
}
async function reloadLatestCloudVersion(){
 if(!state.convoy.date||!state.convoy.shareToken||!window.CPG_GOOGLE?.isConfigured?.())return;
 try{setCloudSyncStatus('Pobieram najnowszą wersję z Google…','saving');const result=await window.CPG_GOOGLE.getConvoy(state.convoy.date,state.convoy.shareToken);await loadConvoyPayload(result.data,{provider:'google',token:state.convoy.shareToken,revision:result.meta?.revision,lastWriterDeviceId:result.meta?.lastWriterDeviceId});toast('Wczytano najnowszą wersję konwoju.')}catch(e){alert(e.message)}
}
$('reloadCloudBtn')?.addEventListener('click',reloadLatestCloudVersion);
...
window.addEventListener('online',async()=>{
  if(state.mode==='executor'&&state.route?.length&&state.convoy?.cloudProvider){
    setCloudSyncStatus('Połączenie wróciło — sprawdzam blokadę i synchronizuję dane…','saving');
    if(!isCompletedConvoy())await claimExecutorLease();
    if(!state.leaseBlocked)flushCloudSave().then(ok=>{if(ok&&isCompletedConvoy()&&$('reportView')?.classList.contains('active'))renderCompletedReportView({reopened:true})});
  }
});
window.addEventListener('offline',()=>{if(state.mode==='executor'&&state.route?.length)setCloudSyncStatus('Brak Internetu — kolejne zmiany zostaną zapisane na telefonie.','offline')});
setDefaultDates();
...
async function initializeRoleView(){
 if(APP_ROLE==='planner'){
   state.mode='planner';
   if(state.route?.length&&!state.routeMeta?.startedAt){renderRoute();showView('routeView')}else showView('setupView');
   return;
 }
 if(APP_ROLE==='executor'){
   state.mode='executor';
   const shared=await autoLoadSharedConvoyFromUrl();if(shared)return;
   if(state.route?.length&&state.convoy?.shareToken){renderRoute();if(isCompletedConvoy()){renderCompletedReportView({reopened:true});showView('reportView')}else{showView('routeView');await claimExecutorLease()}}else showView('convoyLoadView');
   return;
 }
 showView('homeView');autoLoadSharedConvoyFromUrl();
}
initializeRoleView();

// --- OneDrive / Microsoft 365 pilot workflow ---
function sessionPayload(){
  return {schema:'cpg-inkasacja-v8', exportedAt:new Date().toISOString(), convoy:state.convoy, sourceMeta:state.sourceMeta, start:state.start, plan:state.plan, route:state.route, routeMeta:state.routeMeta, history:state.history, sealSequence:state.sealSequence, reportSnapshot:state.reportSnapshot};
}
...
async function importSessionFile(file){
  try{
    ...
    if(!['cpg-inkasacja-v1','cpg-inkasacja-v2','cpg-inkasacja-v3','cpg-inkasacja-v4','cpg-inkasacja-v5','cpg-inkasacja-v6','cpg-inkasacja-v7','cpg-inkasacja-v8'].includes(data.schema)||!Array.isArray(data.route)) throw new Error('Nieprawidłowy format pliku CPG Inkasacja.');
